...
export default function Home() {
 ...
 const total = useMemo(() => ({ borrowers: monthly.at(-1)!.borrowers, visitors: monthly.at(-1)!.visitors, views: monthly.at(-1)!.views }), [])
 return <main>
  <div className="shell">
   <header className="topbar"><div><div className="eyebrow">PUSAT SUMBER & FOTOGRAFI</div><h1>Dashboard Interaktif</h1><p>Jom tengok bagaimana komuniti kita menggunakan perkhidmatan dan koleksi digital.</p></div><div className="controls"><span><CalendarDays size={16}/> 2026</span><select value={period} onChange={e=>setPeriod(e.target.value)}><option>8 Bulan</option><option>6 Bulan</option><option>3 Bulan</option></select></div></header>
   <section className="hero"><div><span className="pill">● LIVE STATISTIK</span><h2>Sehingga hari ini, <em>ramai yang datang.</em></h2><p>Ringkasan prestasi Pusat Sumber dan MyFoto dalam satu paparan yang mudah difahami.</p></div><div className="hero-mark"><Camera size={54}/><strong>MyFoto</strong><small>Arkib foto digital</small></div></section>
   <section className="cards">
    <Stat icon={<BookOpen/>} label="Peminjam" value={total.borrowers.toLocaleString()} delta="+18.4%" note="bulan ini" />
    <Stat icon={<Users/>} label="Pengunjung" value={total.visitors.toLocaleString()} delta="+9.7%" note="bulan ini" />
    <Stat icon={<Eye/>} label="View MyFoto" value={total.views.toLocaleString()} delta="+24.6%" note="bulan ini" />
   </section>
   ...
   <footer>Dashboard statistik awam • Pusat Sumber dan Fotografi • Data contoh untuk demonstrasi</footer>
  </div>
 </main>
}
